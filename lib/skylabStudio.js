...
//   params = {}
// ) {
//   let url = `${API_PROTOCOL}://${API_HOST}/api/public/v${API_VERSION}/`;
...
// SkylabStudio.prototype._handleResponse = function _handleResponse(
//   result,
//   response,
//   callback
// ) {
...
// SkylabStudio.prototype._handleClientError = function _handleResponse(
//   result,
//   response,
//   callback
// ) {
...
// SkylabStudio.prototype._handleSuccess = function _handleResponse(
//   result,
//   response,
//   callback
// ) {
...
// SkylabStudio.prototype._handleServerError = function _handleResponse(
//   result,
//   response,
//   callback
// ) {
...
SkylabStudio.prototype._handleResponse = function _handleResponse(
  ...
  if (result instanceof Error) {
    this._handleClientError(result, status, callback);
  } else if (status >= 200 && status < 400) {
    this._handleSuccess(result, status, callback);
  } else {
    this._handleServerError(result, status, callback);
  }
};
...
SkylabStudio.prototype._handleSuccess = function _handleResponse(
  ...
  this.emit("response", status, result, status);
  this._debug(`Response ${status}: ${JSON.stringify(result)}`);
...
SkylabStudio.prototype._handleServerError = function _handleResponse(
  ...
  const err = new Error(`Server request failed with ${status}`);
