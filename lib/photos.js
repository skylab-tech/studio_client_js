...
module.exports = function photos(SkylabStudio) {
  ...
  SkylabStudio.prototype.downloadPhoto = async function downloadPhoto(
    ...
    if (isExtract) {
      if (isDualFileOutput) {
        console.log("is dual file output");
      ...
      if (type === "image/webp") {
        ...
        console.log("bruh", resCode);
        if (resCode !== 200) return { status: 422, id: photoId };
      } else {
  ...
  SkylabStudio.prototype.downloadPhotos = async function downloadPhotos(
    ...
    const { results, errors } = await PromisePool.withConcurrency(5)
      .for(photoIds)
      .process(async (photoId, index, pool) => {
        console.log(photoId, "inside pool");
        const what = await this.downloadPhoto(photoId, outputPath, profile);
